...
export function RepositoryScanner() {
    ...
    useEffect(() => {
        if (accessToken) loadRepositories();
    }, [accessToken]);

    // Elapsed seconds ticker shown while scanning
    useEffect(() => {
    ...
    const handleScan = async () => {
        ...
        setScanProgress(5);
        setProgressLabel('Cloning repository…');
        const startTime = Date.now();

        // Smooth progress simulation for repo scans (can take 2–3 min)
        let tick = 0;
        const progressInterval = setInterval(() => {
            tick++;
            setScanProgress(prev => {
                if (prev < 20) return Math.min(20, prev + 3);
                if (prev < 45) return Math.min(45, prev + 1.5);
                if (prev < 70) return Math.min(70, prev + 0.7);
                if (prev < 85) return Math.min(85, prev + 0.3);
                return prev;
            });
            if (tick === 4) setProgressLabel('Discovering source files…');
            if (tick === 12) setProgressLabel('Running rule-based analysis…');
            if (tick === 25) setProgressLabel('Running ML vulnerability scan…');
            if (tick === 50) setProgressLabel('Computing risk scores…');
            if (tick >= 80) setProgressLabel('Finalizing report…');
        }, 1000);

        try {
            console.log('[SCAN] Starting repository scan...', { repoUrl, branch: selectedBranch, model: selectedModel });
    ...
    /**
     * Severity color — correct mapping per spec:
     * Critical → Red, High → Orange, Medium → Yellow/Amber, Low → Teal/Green
     */
    const getSeverityColor = (severity: string) => {
        switch (severity?.toLowerCase()) {
            ...
            case 'low': return 'bg-gradient-to-r from-teal-400 to-emerald-500';   // ← was blue
            default: return 'bg-gray-500';
        }
    };

    /**
     * Risk level mapping — Low now uses teal (not blue) to avoid confusion with info.
     */
    const getRiskLevel = (score: number) => {
        ...
        if (score > 0) return { label: 'Low', color: 'text-teal-400', gradient: 'from-teal-400 to-emerald-500' }; // ← was blue
                ...
                <Card className="bg-white/40 dark:bg-white/5 backdrop-blur-lg border-slate-200 dark:border-white/10 shadow-2xl">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-slate-700 dark:text-white/80">Repository URL</label>
                            <Input
                        ...
                        {user && repositories.length > 0 && (
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700 dark:text-white/80">Or select from your repositories</label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700 dark:text-white/80 flex items-center gap-2">
                                    <Shield className="h-4 w-4" /> AI Model
                                </label>
                        ...
                        >
                            {scanning ? (
                                <>
                                    ...
                                    Scanning Repository… ({elapsedSeconds}s elapsed)
                                </>
                            ) : (
                                <>
                        ...
                        <AnimatePresence>
                            {scanning && (
                                ...
                                >
                                    <div className="flex justify-between text-sm text-slate-700 dark:text-white/80">
                                        <span className="flex items-center gap-2">
                                            <Zap className="w-4 h-4 text-cyan-400 animate-pulse" />
                                            {progressLabel || 'Scanning repository with parallel processing…'}
                                        </span>
                                        <span className="font-mono">{Math.round(scanProgress)}%</span>
                                    </div>
                                    <Progress value={scanProgress} className="h-2 bg-slate-200 dark:bg-white/10" />
                                    <p className="text-xs text-slate-500 dark:text-white/40 text-center">
                                        Large repositories may take 2–3 minutes. Please keep this tab open.
                                    </p>
                                </motion.div>
                            )}
                        </AnimatePresence>
                            ...
                            {!scanning && scanError && (
                                ...
                                >
                                    <Alert className="bg-red-500/10 border-red-500/40">
                                        ...
                                        <AlertTitle className="text-red-400 font-semibold">Scan Failed</AlertTitle>
                                        <AlertDescription className="text-sm text-slate-700 dark:text-white/80 mt-1 flex flex-col gap-3">
                                            <span>{scanError}</span>
                                            <Button
                                                size="sm"
                                                onClick={handleScan}
                                                disabled={!repoUrl}
                                                className="self-start bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/40"
                                                variant="outline"
                                            >
                        ...
                        <Card className="bg-white/5 backdrop-blur-lg border-white/10 shadow-2xl overflow-hidden">
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <CardDescription className="text-slate-600 dark:text-white/60">
                                            ...
                                            {scanDuration ? ` in ${scanDuration.toFixed(1)}s` : ''}
                                        </CardDescription>
                                    </div>
                                    <Button
                                        onClick={() => {
                                            try {
                                                ...
                                                generatePDFReport(
                                                    { riskScore: scanResult.overall_risk_score || 0, vulnerabilities, linesScanned: scanResult.scanned_files || 0, timeElapsed: 0 },
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                                    ...
                                    {/* Severity label — dynamic color instead of hardcoded blue */}
                                    <div className="text-center p-4 bg-slate-800/40 backdrop-blur-sm rounded-xl border border-white/10">
                                        <Activity className="w-5 h-5 mx-auto mb-1 text-white/50" />
                                        <div className={`text-4xl font-bold ${getRiskLevel(scanResult.overall_risk_score || 0).color}`}>
                                    ...
                                    <div className="text-center p-4 bg-red-500/10 backdrop-blur-sm rounded-xl border border-red-500/20">
                                        ...
                                        <div className="text-4xl font-bold text-red-400">{allConfirmedFindings.length}</div>
                                ...
                                <div className="flex items-center justify-center gap-1 py-3 px-4 bg-black/40 backdrop-blur-sm rounded-lg border border-white/10 text-sm">
                                    ...
                                    <span className="text-white/20 mx-2">|</span>
                                    <div className="flex items-center gap-1.5">
                                        <span className="font-bold text-teal-400">Low</span>
                                ...
                                {/* ── Clean / No Confirmed Vuln messages ── */}
                                {scanResult.overall_risk_score === 0 && (
                                    <Alert className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border-green-500/30">
                                        ...
                                        <AlertTitle className="text-green-400">No vulnerabilities detected. Repository appears secure.</AlertTitle>
                                        <AlertDescription className="text-slate-700 dark:text-white/80">
                                            No security issues were found in this repository.
                                        </AlertDescription>
                                    </Alert>
                                )}

                                {/* ML detected risk but no confirmed rule-based findings */}
                                {scanResult.overall_risk_score > 0 && allConfirmedFindings.length === 0 && (
                                    <Alert className="bg-amber-500/10 border-amber-500/30">
                                        <AlertCircle className="h-4 w-4 text-amber-400" />
                                        <AlertTitle className="text-amber-400">No confirmed vulnerabilities found</AlertTitle>
                                        <AlertDescription className="text-slate-700 dark:text-white/80">
                                            The ML model detected some risk patterns (score: {scanResult.overall_risk_score.toFixed(1)}) but no specific rule-based vulnerabilities were confirmed. Manual review is recommended.
                                        </AlertDescription>
                                    </Alert>
                                )}
                            </CardContent>
                                ...
                                <CardContent>
                                    <Accordion type="single" collapsible className="space-y-2">
                                        {scanResult.file_results
                                            ...
                                            .map((file, index) => {
                                                // Only show severity badge when there are confirmed findings
                                                const hasFindings = Array.isArray(file.highlights) && file.highlights.length > 0;
                                                return (
                                                    <AccordionItem
                                                        key={index}
                                                        value={`file-${index}`}
                                                    ...
                                                    >
                                                        <AccordionTrigger className="hover:no-underline">
                                                            <div className="flex items-center gap-3 flex-1">
                                                                ...
                                                                {/* Only show severity badge if there are actual findings */}
                                                                {hasFindings && file.severity && (
                                                        ...
                                                        <AccordionContent className="space-y-3 pt-3">
                                                            {hasFindings && (
                                                                <div className="space-y-2">
                                                                    ...
                                                                    {file.highlights!.map((vuln: any, vIdx: number) => (
                                                                        <div
                                                                            key={vIdx}
                                                                            className={`p-3 rounded-lg border flex flex-col gap-1 ${vuln.severity?.toUpperCase() === 'CRITICAL' ? 'bg-red-500/10 border-red-500/30' :
                                                                                    vuln.severity?.toUpperCase() === 'HIGH' ? 'bg-orange-500/10 border-orange-500/30' :
                                                                                        vuln.severity?.toUpperCase() === 'MEDIUM' ? 'bg-yellow-500/10 border-yellow-500/30' :
                                                                                            'bg-teal-500/10 border-teal-500/30'
                                                                                }`}
                                                                        >
                                                                            <div className="flex items-center justify-between gap-2 flex-wrap">
                                                                                <div className="flex items-center gap-2">
                                                                                    <Badge className={`${vuln.severity?.toUpperCase() === 'CRITICAL' ? 'bg-red-500' :
                                                                                            vuln.severity?.toUpperCase() === 'HIGH' ? 'bg-orange-500' :
                                                                                                vuln.severity?.toUpperCase() === 'MEDIUM' ? 'bg-amber-500' :
                                                                                                    'bg-teal-500'
                                                                                        } text-white border-0 text-[10px] px-1.5 h-4`}>
                                                                                        {vuln.severity}
                                                                                    </Badge>
                                                                                    <span className="text-xs font-bold text-slate-900 dark:text-white">
                                                                                        {vuln.line ? `Line ${vuln.line}: ` : ''}{vuln.type}
                                                                                    </span>
                                                                                </div>
                                                                                {vuln.confidence !== undefined && (
                                                                                    <span className="flex items-center gap-1 text-[10px] font-semibold text-slate-500 dark:text-white/40 bg-white/5 px-2 py-0.5 rounded-full border border-white/10">
                                                                                        Confidence: <span className="text-emerald-400 ml-0.5">{Math.round(vuln.confidence * 100)}%</span>
                                                                                    </span>
                                                                                )}
                                                                            </div>
                                                                            {vuln.content && (
                                                                            ...
                                                                            {vuln.description && (
                                                                                <div className="text-xs text-slate-600 dark:text-slate-400 italic">{vuln.description}</div>
                                                                            )}
                                                                            {vuln.fix && (
                                                            ...
                                                            {file.suggested_fix && (!Array.isArray(file.highlights) || !file.highlights.some((v: any) => v.fix)) && (
